...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Find winners by phone_e164 (and optionally action_id)
    ...
    if (!matched || matched.length === 0) {
      // Fallback: if no action_id provided, try finding by phone_e164 alone with operational status
      if (action_id) {
        // action_id was provided but no match — try without action_id as fallback
        ...
        if (fallbackMatched && fallbackMatched.length > 0) {
          return await processInbound(serviceClient, fallbackMatched, phoneE164);
        }
      }
    ...
    return await processInbound(serviceClient, matched, phoneE164);
  } catch (err) {
