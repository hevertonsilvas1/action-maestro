...
export default function NewActionPage() {
  ...
  // Access control
  if (roleLoading || ptLoading || ctLoading) {
    return (
      <AppLayout>
    ...
    return (
      <AppLayout>
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <p className="text-muted-foreground">Você não tem permissão para criar ações.</p>
        </div>
      </AppLayout>
    );
  }
  ...
  async function handleSave(forceNegative = false) {
    ...
    if (grossProfit < 0 && !forceNegative) {
      setShowNegativeProfitAlert(true);
      return;
    }

    const prizesInput: PrizeInput[] = prizes.map(p => ({
      prizeTypeConfigId: p.typeConfigId,
      title: p.title,
      description: p.description || undefined,
      quantity: parseInt(p.quantity),
      unitValue: parseFloat(p.unitValue),
      totalValue: parseInt(p.quantity) * parseFloat(p.unitValue),
    }));

    const costsInput: CostInput[] = costs.map(c => ({
      costTypeConfigId: c.typeConfigId,
      description: c.description,
      quantity: parseInt(c.quantity),
      unitValue: parseFloat(c.unitValue),
    ...
    <AppLayout>
      ...
      <div className="flex-1 overflow-auto p-4 lg:p-6 space-y-6 max-w-4xl">
        ...
        <section className="rounded-xl border bg-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <Input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Ação Natal 2025" />
        ...
        <section className="rounded-xl border bg-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <Select value={quotaCount?.toString() ?? ''} onValueChange={v => setQuotaCount(parseInt(v))}>
                <SelectTrigger><SelectValue placeholder="Selecionar..." /></SelectTrigger>
                <SelectContent className="max-h-60">
                  <SelectGroup>
                    <SelectLabel>Mais utilizadas</SelectLabel>
                    {QUOTA_OPTIONS_POPULAR.map(q => (
                      <SelectItem key={`pop-${q}`} value={q.toString()}>
                        {fmtQuota(q)}
                      </SelectItem>
                    ))}
                  </SelectGroup>
        ...
        <section className="rounded-xl border bg-card p-4 space-y-4">
          ...
          {prizes.length === 0 && (
            <p className="text-xs text-muted-foreground">Nenhuma premiação adicionada.</p>
          )}
            ...
            return (
              <div key={i} className="grid grid-cols-1 sm:grid-cols-6 gap-3 p-3 rounded-lg border bg-muted/30">
                <div className="sm:col-span-2">
                  ...
                  <Select value={p.typeConfigId} onValueChange={v => {
                    const arr = [...prizes]; arr[i].typeConfigId = v; setPrizes(arr);
                  }}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Tipo..." /></SelectTrigger>
                    <SelectContent>
                      {activePrizeTypes.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="sm:col-span-2">
                  ...
                  <Input className="h-8 text-xs" value={p.title} onChange={e => {
                    const arr = [...prizes]; arr[i].title = e.target.value; setPrizes(arr);
                  }} />
                  {errors[`prize_title_${i}`] && <p className="text-xs text-destructive">{errors[`prize_title_${i}`]}</p>}
                </div>
                <div>
                  <Label className="text-xs">Qtd *</Label>
                  <Input className="h-8 text-xs" type="number" min="1" value={p.quantity} onChange={e => {
                    const arr = [...prizes]; arr[i].quantity = e.target.value; setPrizes(arr);
                  }} />
                </div>
                <div>
                  <Label className="text-xs">Valor unit. *</Label>
                  <Input className="h-8 text-xs" type="number" step="0.01" min="0" value={p.unitValue} onChange={e => {
                    const arr = [...prizes]; arr[i].unitValue = e.target.value; setPrizes(arr);
                  }} />
                </div>
                <div className="sm:col-span-4">
                  <Label className="text-xs">Descrição</Label>
                  <Input className="h-8 text-xs" value={p.description} onChange={e => {
                    const arr = [...prizes]; arr[i].description = e.target.value; setPrizes(arr);
          ...
          {prizes.length > 0 && (
            <div className="text-right text-sm font-semibold">
              Total Premiações: {formatCurrency(totalPrizes)}
            </div>
          )}
        </section>
        ...
        <section className="rounded-xl border bg-card p-4 space-y-4">
          ...
          {costs.length === 0 && (
            <p className="text-xs text-muted-foreground">Nenhum custo adicionado.</p>
          )}
          ...
          {costs.map((c, i) => {
            ...
            return (
              <div key={i} className="grid grid-cols-1 sm:grid-cols-6 gap-3 p-3 rounded-lg border bg-muted/30">
                <div className="sm:col-span-2">
                  <Label className="text-xs">Tipo *</Label>
                  <Select value={c.typeConfigId} onValueChange={v => {
                    const arr = [...costs]; arr[i].typeConfigId = v; setCosts(arr);
                  }}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Tipo..." /></SelectTrigger>
                    <SelectContent>
                      {activeCostTypes.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="sm:col-span-2">
                  <Label className="text-xs">Descrição *</Label>
                  <Input className="h-8 text-xs" value={c.description} onChange={e => {
                    const arr = [...costs]; arr[i].description = e.target.value; setCosts(arr);
                  }} />
                  {errors[`cost_desc_${i}`] && <p className="text-xs text-destructive">{errors[`cost_desc_${i}`]}</p>}
                </div>
                <div>
                  <Label className="text-xs">Qtd *</Label>
                  <Input className="h-8 text-xs" type="number" min="1" value={c.quantity} onChange={e => {
                    const arr = [...costs]; arr[i].quantity = e.target.value; setCosts(arr);
                  }} />
                </div>
                <div>
                  <Label className="text-xs">Valor unit. *</Label>
                  <Input className="h-8 text-xs" type="number" step="0.01" min="0" value={c.unitValue} onChange={e => {
                    const arr = [...costs]; arr[i].unitValue = e.target.value; setCosts(arr);
                  }} />
                </div>
          ...
          {costs.length > 0 && (
            <div className="text-right text-sm font-semibold">
              Total Custos: {formatCurrency(totalCosts)}
            </div>
          )}
        </section>
        ...
        <section className="rounded-xl border bg-card p-4 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Custos manuais de legalização/impostos podem ser adicionados na seção de Custos usando o tipo correspondente.
          </p>
        </section>
        ...
        <section className={`rounded-xl border p-4 space-y-3 ${grossProfit < 0 ? 'border-destructive bg-destructive/5' : 'bg-card'}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-sm">
            ...
            <div>
              ...
              <p className={`font-bold ${grossProfit < 0 ? 'text-destructive' : 'text-success'}`}>
                {formatCurrency(grossProfit)}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Margem</p>
              <p className={`font-bold ${marginPercent < 0 ? 'text-destructive' : 'text-success'}`}>
          ...
          {grossProfit < 0 && (
            <div className="flex items-center gap-2 text-destructive text-xs font-medium">
              ...
              Atenção: O lucro está negativo! Revise os custos e premiações.
            </div>
          )}
        </section>
        ...
        <div className="flex justify-end gap-3 pb-8">
          ...
          >
            {createAction.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}
            Salvar Ação
          </Button>
        </div>
      </div>

      {/* New prize type dialog */}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Crie um novo tipo de premiação que ficará disponível para todas as ações.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Input value={newPrizeTypeName} onChange={e => setNewPrizeTypeName(e.target.value)} placeholder="Ex: Giro Abençoado" />
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleCreatePrizeType} disabled={!newPrizeTypeName.trim()}>
              Criar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={showNegativeProfitAlert} onOpenChange={setShowNegativeProfitAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              O lucro bruto planejado é <strong>{formatCurrency(grossProfit)}</strong> (margem: {formatPercent(marginPercent)}).
              Tem certeza que deseja salvar esta ação com prejuízo?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Revisar</AlertDialogCancel>
            <AlertDialogAction onClick={() => { setShowNegativeProfitAlert(false); handleSave(true); }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Salvar mesmo assim
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
