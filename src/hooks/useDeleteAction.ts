...
export function useDeleteAction() {
  ...
  const mutation = useMutation({
    mutationFn: async (actionId: string) => {
      ...
      // Fetch action name before deleting
      ...
      const actionName = action?.name || 'Desconhecida';
      ...
      await insertAuditLog({
        ...
        recordId: actionId,
        operation: 'delete',
        changes: { 'Ação excluída': actionName },
      });
      ...
      // Note: audit log entries are preserved for historical record
